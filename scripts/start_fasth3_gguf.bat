@echo off
setlocal EnableDelayedExpansion
rem FastH3 GGUF Q4: ComfyUI (8188) + /v1/videos sidecar (8000).

call "%~dp0_env.bat"
if not defined COMFY_ROOT (
  echo 缺少 COMFY_ROOT。请复制 scripts\paths.example.bat 为 scripts\paths.bat 并填写。
  if /i not "%AISHOW_HEADLESS%"=="1" pause
  exit /b 1
)
if not defined FASTH3_GGUF_ROOT (
  echo 缺少 FASTH3_GGUF_ROOT / MODELS_ROOT。
  if /i not "%AISHOW_HEADLESS%"=="1" pause
  exit /b 1
)

set "FASTH3_HOST=127.0.0.1"
set "FASTH3_PORT=8000"
set "FASTH3_COMFY_URL=http://127.0.0.1:8188"
set "PYTHONUNBUFFERED=1"
if defined AISHOW_TMP if not exist "%AISHOW_TMP%" mkdir "%AISHOW_TMP%"
if defined TRITON_CACHE_DIR if not exist "%TRITON_CACHE_DIR%" mkdir "%TRITON_CACHE_DIR%"

if not exist "%COMFY_PY%" (
  echo 找不到 ComfyUI Python: %COMFY_PY%
  if /i not "%AISHOW_HEADLESS%"=="1" pause
  exit /b 1
)
if not exist "%FASTH3_GGUF_ROOT%\diffusion_models\FastH3-comfy-Q4_K_M.gguf" (
  echo 找不到 FastH3 Q4 GGUF: %FASTH3_GGUF_ROOT%
  echo 请先跑 scripts\download_fasth3_gguf.ps1
  if /i not "%AISHOW_HEADLESS%"=="1" pause
  exit /b 1
)

curl.exe -s -o nul -m 3 http://127.0.0.1:8188/system_stats
if not errorlevel 1 goto comfy_ok

echo 启动 ComfyUI  -^>  http://127.0.0.1:8188
start "FastH3 ComfyUI" /D "%COMFY_ROOT%" "%COMFY_PY%" -s ComfyUI\main.py --windows-standalone-build --listen 127.0.0.1 --port 8188 --fast fp16_accumulation
echo 等待 ComfyUI 就绪...
set /a _n=0
:wait_comfy
timeout /t 2 /nobreak >nul
curl.exe -s -o nul -m 3 http://127.0.0.1:8188/system_stats
if not errorlevel 1 goto comfy_ok
set /a _n+=1
if !_n! GEQ 90 (
  echo ComfyUI 90 秒还没起来，请看「FastH3 ComfyUI」窗口。
  if /i not "%AISHOW_HEADLESS%"=="1" pause
  exit /b 1
)
goto wait_comfy

:comfy_ok
echo ComfyUI 已就绪  -^>  http://127.0.0.1:8188
echo FastH3 GGUF 边车  -^>  http://127.0.0.1:8000
echo 工坊选「FastH3 本地」，推理模式 auto。
echo.
cd /d "%AISHOW_ROOT%"
"%COMFY_PY%" "%AISHOW_ROOT%\backend\python\fasth3_gguf_server.py"
echo.
echo FastH3 GGUF 边车已退出。
if /i not "%AISHOW_HEADLESS%"=="1" pause
