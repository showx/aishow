...
边车地址：`AISHOW_SGLANG_FL2VA_URL`（30010）、`AISHOW_SGLANG_REF2VA_URL`（30011）、`AISHOW_FASTH3_URL`（8000）、`AISHOW_H3_TURBO_URL`（30012）、`AISHOW_H3_PINKCHERRY_URL`（30013）、`AISHOW_H3_DIRECTOR_URL`（30014）、`AISHOW_LLADA_IMAGE_URL`（30020）、`AISHOW_QWEN_IMAGE_URL`（30021）。

本地 Chat（短剧写剧本 / 拆分镜）：`AISHOW_CHAT_URL`（默认 `http://127.0.0.1:11434`）、`AISHOW_CHAT_MODEL`（默认 `qwen3.5:4b`）、`AISHOW_CHAT_API_TOKEN`（Ollama 通常留空）。会跳过 `qwen3-vl` 这类视觉模型。不是 H3 的 Qwen3-VL 文本编码器。启动：`scripts\start_drama_chat.bat`。

本地 TTS（可选，合成时叠对白）：`AISHOW_TTS_URL`、`AISHOW_TTS_MODEL`、`AISHOW_TTS_VOICE`、`AISHOW_TTS_API_TOKEN`。空地址则只用 H3 自己的音轨。
