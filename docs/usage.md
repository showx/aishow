...
- **角色 / 场景参考**：剧本、出图、成片步都可以上传人物或场景图。出图默认 **Qwen-Image-2.1**，参考图最多 10 张一起进指令编辑；改回 LLaDA-Image 时只带第一张，其余名字写进提示。成片选 Director / Ref2VA 时，这些图会和本镜出图一起送进模型（最多 9 张）。每一镜还可以再加本镜额外参考。
- 续写引擎选 H3 / Turbo / PinkCherry 时，会抽上一镜**尾帧**当本镜首帧，并能和本镜出图做首尾桥接。HunyuanVideo-1.5 / LTX-2.3 也抽尾帧，但只吃这一张首帧。Ref2VA / Director 仍用上一镜成片续写，并继续带上角色 / 场景参考图。
