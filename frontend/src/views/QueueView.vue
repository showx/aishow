    ...
    <div class="board">
      ...
      <section class="col panel">
        ...
        <article v-for="job in done" :key="job.id" class="card">
          ...
          <div class="meta">{{ job.stage }}</div>
          <p v-if="job.error_message" class="err">{{ job.error_message }}</p>
          <div class="ops">
...
.col { padding: 18px; min-height: 420px; }
...
.meta, .empty, .stage { color: var(--muted); font-size: 12px; margin: 6px 0; }
.pos { color: var(--gold); }
.pct { color: var(--mint); }
p { color: var(--muted); font-size: 13px; line-height: 1.6; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
